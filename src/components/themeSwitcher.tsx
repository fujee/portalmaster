...
export default function ThemeSwitcher({ changeTextColor, callback }: { changeTextColor?: boolean, callback?: () => void }) {
    ...
    return (
        <label className="inline-flex items-center cursor-pointer ms-3 lg:ms-0 mb-4 lg:mb-0">
            ...
            <span className={`ms-3 text-sm  ${changeTextColor ? 'text-secondary' : 'text-white'}`}>{theme === DARK_THEME ? t('Светла тема') : t('Тамна тема')}</span>
        </label>
    );
}