            ...
            <div className="min-h-screen relative">
                ...
                <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full px-5 lg:px-0 lg:max-w-screen-lg'>
                    <div className='block border-2 border-primary bg-white/10 z-10 py-2 px-4 rounded-md text-sm lg:text-lg mt-18 lg:mt-0 text-white w-fit mx-auto'>
                        {t("Једна платформа - бескрајне могућности", "welcome")}
                    </div>

                    <h1 className="text-3xl lg:text-7xl text-white text-center mt-8 font-bold uppercase">
                        {t('ПОРТАЛ ')}
                        <span className='text-primary relative'>{t('НА КЛИК')}
                        </span>
                        {t(' ОД ВАС')}
                    </h1>
                    <h2 className='text-lg lg:text-2xl text-white text-center uppercase mt-2'>{t('ЛАКО, БЕЗБЕДНО И ЕФИКАСНО')}</h2>
                    ...
                    <div className='flex justify-center items-center mt-20 gap-4'>
                        <a href='mailto:contact@portalmastersolutions.com' className='py-2 px-4 text-white bg-primary border-2 border-primary rounded-md hover:bg-primary/30 cursor-pointer ease-in-out duration-200'>{t("Започни сада")}</a>
                        <Link to='about'
                            smooth={true}
                            spy={true}
                            duration={500}
                            className='py-2 px-4 text-white bg-transparent border-2 border-white rounded-md hover:bg-white/30 cursor-pointer ease-in-out duration-200'>{t("Сазнај више")} <FontAwesomeIcon icon={faArrowRight} className='ms-2' /></Link>
                    </div>
                </div>
            </div>
        </Element>);
